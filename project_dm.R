...
ctrl =trainControl(method="cv", number = 10, classProbs = T,
                   summaryFunction=twoClassSummary, )
grdi <- expand.grid(laplace=c(0,1), usekernel = F, adjust=0)
...
library(naivebayes)
naive_all4 <- NaiveBayes(as.factor(venditore) ~ ., data = train.df, laplace = 1)
naive_all4
...
library(stats)
set.seed(1)
contr=trainControl(method= "cv",number=10, classProbs=TRUE,
                   summaryFunction=twoClassSummary)
pca=train(venditore~. , data=train.df , method = "pca", 
          trControl = contr, tuneLength=5, metric="Spec")
pca
plot(pca)
confusionMatrix(pca)
...
save(tree, rf, glm, glm_cor, bagging, lasso, boost1, knn_scale, knn_boot, knn_pc, nnet_grid,
     pls, naivebayes,  naivebayes_sel, file = "modelli.RData")

results <- resamples(list(tree=tree, rf=rf, glm=glm, glm_cor=glm_cor, bagging=bagging, lasso=lasso, boost1=boost1,
                          pls=pls, naivebayes=naivebayes, naivebayes_sel= naivebayes_sel))
...
pred <- ifelse(score.df$rf > 0.20, "c1", "c0")
table(actual=score.df$venditore, pred)

library(caret)
confusionMatrix(as.factor(pred), as.factor(score.df$venditore), positive="c1")

nb <- predict(naive_all4, train.df, "prob")[,2]
